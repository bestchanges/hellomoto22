...
class Currency(db.Document):
    ...
    algo = db.StringField(max_length=50, choices=ALGORITHMS)
...
class MinerProgram(db.Document):
    name = db.StringField(max_length=100)  # human readable miner name
    code = db.StringField(max_length=50, unique=True)  # unique id
    family = db.StringField(max_length=50)  # family has the same output handlers
    command_line = db.StringField(max_length=500)
    dir = db.StringField(max_length=100)
